...
# change the variable for the path to hpctoolkit
hpctoolkit = "/home/la5/pkgs/hpctoolkit-fix"
hpcrun     = "#{hpctoolkit}/bin/hpcrun"

min_cpus = read_int_param("--min-cpus", 1)
max_cpus = read_int_param("--max-cpus", 70)
repeats = read_int_param("--repeats", 5)
timeout_secs = read_int_param("--timeout", 1000)

launch_types = %w(deferred async)
	...
	launch_types.each do |launch_type|
		...
		while(num_cpus <= max_cpus)
			...
			if(OS.windows?)
				...
				command = "timeout #{timeout_secs} taskset -c #{cpulist} #{hpcrun} -o hpctoolkit-#{fname}.#{launch_type}.#{num_cpus}-measurements -t -e cycles #{binfname}"
